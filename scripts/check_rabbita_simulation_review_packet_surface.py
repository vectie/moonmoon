#!/usr/bin/env python3
"""Execute Rabbita UI and check MoonRobo simulation review packet evidence."""
...
const rows = document.getElementById('simulation-review-packet').children.map(row => ({
...
"""
  with tempfile.TemporaryDirectory(prefix="moonmoon-rabbita-sim-packet-ui-") as tmp:
    tmp_dir = Path(tmp)
    harness_path = tmp_dir / "rabbita_sim_packet_harness.cjs"
...
def main() -> int:
  ...
  if "Simulation Review Packet" not in html:
    raise AssertionError("missing simulation review packet section")
  if "function renderSimulationReviewPacket()" not in html:
    raise AssertionError("missing simulation packet renderer")
  ...
  print("checked Rabbita simulation review packet surface")
  return 0
