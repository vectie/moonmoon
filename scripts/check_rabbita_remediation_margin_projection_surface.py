#!/usr/bin/env python3
"""Execute Rabbita UI and check MoonRobo remediation-margin projection evidence."""
...
ENTRY_KIND = "MoonroboRemediationMarginProjection"
...
const rows = document.getElementById('remediation-margin-projection').children.map(row => ({
...
"""
  with tempfile.TemporaryDirectory(
    prefix="moonmoon-rabbita-remediation-projection-ui-"
  ) as tmp:
    tmp_dir = Path(tmp)
    harness_path = tmp_dir / "rabbita_remediation_projection_harness.cjs"
...
def assert_workspace_payload() -> None:
  ...
  if set(payload["consumed_margin_result_ids"]) != set(MARGIN_IDS):
...
def assert_surface(rendered: dict[str, Any]) -> None:
  ...
  if "remediation-margin-projection-row" not in row["className"]:
    raise AssertionError(row)
...
def main() -> int:
  ...
  if "Remediation Margin Projection" not in html:
    raise AssertionError("missing remediation margin projection section")
  if "function renderRemediationMarginProjection()" not in html:
    raise AssertionError("missing remediation margin projection renderer")
  ...
  print("checked Rabbita remediation margin projection surface")
  return 0
