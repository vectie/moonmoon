#!/usr/bin/env python3
"""Check imported MoonClaw gap receipt materialization in MoonBook workspace."""
...
ENTRY_PATH = "moonclaw/first-trusted-square/moonrobo-gap-receipt.json"
RECEIPT_ID = (
...
def assert_gap_receipt_workspace(root: Path) -> None:
  workspace = root / WORKSPACE
  index = load_json(workspace / "index.json")
  manifest = load_json(workspace / "manifest.json")
  receipt_bundle = load_json(
    root / "output/moonclaw/first_trusted_square_moonrobo_gap_receipt.json",
  )
  receipt_entry = load_json(workspace / ENTRY_PATH)

  entries = {entry["entry_id"]: entry for entry in index["entries"]}
  if ENTRY_ID not in entries:
    raise AssertionError("MoonBook workspace index has no imported gap receipt")
  entry = entries[ENTRY_ID]
  if entry["path"] != ENTRY_PATH:
    raise AssertionError(entry["path"])
  if ENTRY_PATH not in manifest["entry_paths"]:
    raise AssertionError("manifest has no imported gap receipt payload path")

  payload = receipt_entry["payload"]
  if receipt_entry["entry"] != entry:
    raise AssertionError("per-entry wrapper does not preserve the indexed entry")
  if payload["primary_receipt"]["receipt"]["receipt_id"] != RECEIPT_ID:
    raise AssertionError(payload["primary_receipt"]["receipt"]["receipt_id"])
  if payload["receipts"] != receipt_bundle:
    raise AssertionError("workspace payload diverges from generated receipt bundle")
  if payload["primary_receipt"]["hardware_state"] != "HardwareDenied":
    raise AssertionError(payload["primary_receipt"]["hardware_state"])
  if not payload["primary_receipt"]["gap_results"]:
    raise AssertionError("gap receipt lost per-gap results")
  if (
    "output/moonclaw/first_trusted_square_moonrobo_gap_receipt.json"
    not in index["source_files"]
  ):
    raise AssertionError("index source_files does not include imported gap receipt")


def main() -> int:
  with tempfile.TemporaryDirectory(prefix="moonmoon-moonbook-gap-receipt-") as tmp:
    tmp_root = Path(tmp)
    ...
    assert_gap_receipt_workspace(tmp_root)
  return 0
