#!/usr/bin/env python3
"""Check MoonBook workspace materialization for fresh-evidence action receipts."""
...
README_SOURCE = (
  "Source MoonClaw remediation-margin fresh evidence action receipts"
)
TASK_ID = (
...
def assert_index_manifest_readme() -> None:
  ...
  require(SOURCE_FILE in index["source_files"], "action receipts missing source file")
  require(ENTRY_REL_PATH in manifest["entry_paths"], "manifest missing action receipts")
  require(README_SOURCE in readme, "README missing action receipts source")
  entries = {entry["entry_id"]: entry for entry in index["entries"]}
  require(ENTRY_ID in entries, "index missing action receipts entry")
  entry = entries[ENTRY_ID]
  require(entry["kind"] == ENTRY_KIND, "unexpected action receipts entry kind")
  require(entry["path"] == ENTRY_REL_PATH, "unexpected action receipts entry path")
  for token in (
...
def assert_workspace_payload() -> None:
  ...
  for margin_id, execution_mode in EXPECTED_ACTIONS.items():
    ...
    require(result["execution_mode"] == execution_mode, f"{margin_id} mode")
...
def main() -> None:
  ...
  print("checked MoonBook fresh evidence action receipt workspace")
