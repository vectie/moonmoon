#!/usr/bin/env python3
"""Execute Rabbita UI and check MoonClaw refresh follow-up task evidence."""
...
ENTRY_KIND = "MoonClawRemediationMarginRefreshFollowupTask"
...
const rows = document.getElementById('remediation-margin-refresh-followup-task').children.map(row => ({
...
"""
  with tempfile.TemporaryDirectory(
    prefix="moonmoon-rabbita-remediation-refresh-followup-ui-",
  ) as tmp:
    tmp_dir = Path(tmp)
    harness_path = tmp_dir / "rabbita_remediation_refresh_followup_harness.cjs"
...
def assert_workspace_payload() -> None:
  ...
  if primary["blocking_refresh_count"] != 3:
    raise AssertionError(primary["blocking_refresh_count"])
  ...
  actions = primary["followup_actions"]
  if len(actions) != 3:
...
def assert_surface(rendered: dict[str, Any]) -> None:
  ...
  if "remediation-margin-refresh-followup-task-row" not in row["className"]:
    raise AssertionError(row)
...
def main() -> int:
  ...
  if "Remediation Margin Refresh Follow-Up Task" not in html:
    raise AssertionError("missing remediation margin refresh follow-up section")
  if "function renderRemediationMarginRefreshFollowupTask()" not in html:
    raise AssertionError("missing remediation margin refresh follow-up renderer")
  ...
  print("checked Rabbita remediation margin refresh follow-up task surface")
  return 0
