#!/usr/bin/env python3
"""Check selected-route terrain remediation is durable in MoonBook workspace."""
...
ENTRY_ID = "terrain-remediation/first-trusted-square/northeast-stepout"
ENTRY_PATH = (
...
def assert_terrain_workspace(root: Path) -> None:
  ...
  if ENTRY_ID not in entries:
    raise AssertionError("MoonBook workspace index has no terrain remediation entry")
  entry = entries[ENTRY_ID]
  if entry["kind"] != "TerrainRemediationEvidence":
    raise AssertionError(entry)
  if entry["path"] != ENTRY_PATH:
    raise AssertionError(entry["path"])
  if ENTRY_PATH not in manifest["entry_paths"]:
    raise AssertionError("manifest has no terrain remediation payload path")
  if SOURCE_PATH not in index["source_files"]:
    raise AssertionError("index source_files does not include terrain source")
  ...
  if entry_file["payload"] != source:
    raise AssertionError("workspace terrain payload diverges from mission output")
  payload = entry_file["payload"]
  if payload["decision"] != "Block":
    raise AssertionError(payload["decision"])
  if payload["max_neighbor_grade"] <= payload["grade_limit"]:
    raise AssertionError(payload)
  if payload["roughness_m"] <= payload["roughness_limit_m"]:
    raise AssertionError(payload)
  if payload["blocking_edge_count"] <= 0:
    raise AssertionError(payload)


def main() -> int:
  assert_terrain_workspace(ROOT)
  with tempfile.TemporaryDirectory(prefix="moonmoon-moonbook-terrain-") as tmp:
    tmp_root = Path(tmp)
    ...
    assert_terrain_workspace(tmp_root)
  print("checked selected-route terrain MoonBook workspace")
  return 0
