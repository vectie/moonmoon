#!/usr/bin/env python3
"""Execute Rabbita UI and check MoonRobo simulation blocker reduction evidence."""
...
ENTRY_KIND = "MoonroboSimulationBlockerReduction"
...
const rows = document.getElementById('simulation-blocker-reduction').children.map(row => ({
...
"""
  with tempfile.TemporaryDirectory(prefix="moonmoon-rabbita-sim-blocker-ui-") as tmp:
    tmp_dir = Path(tmp)
    harness_path = tmp_dir / "rabbita_sim_blocker_harness.cjs"
...
def assert_workspace_payload() -> None:
  ...
  if payload["hardware_authority"] != "moonmoon-safety-gate-only":
    raise AssertionError(payload)
  if payload["may_consume_after_reduction"] is not False:
    raise AssertionError(payload)
...
def assert_surface(rendered: dict[str, Any]) -> None:
  ...
  if "simulation-blocker-reduction-row" not in row["className"]:
    raise AssertionError(row)
...
def main() -> int:
  ...
  if "Simulation Blocker Reduction" not in html:
    raise AssertionError("missing simulation blocker reduction section")
  if "function renderSimulationBlockerReduction()" not in html:
    raise AssertionError("missing simulation blocker reduction renderer")
  ...
  print("checked Rabbita simulation blocker reduction surface")
  return 0
