#!/usr/bin/env python3
"""Execute Rabbita UI and check MoonClaw remediation-margin refresh receipt evidence."""
...
ENTRY_KIND = "MoonClawRemediationMarginRefreshReceipt"
...
const rows = document.getElementById('remediation-margin-refresh-receipt').children.map(row => ({
...
"""
  with tempfile.TemporaryDirectory(prefix="moonmoon-rabbita-remediation-refresh-receipt-ui-") as tmp:
    tmp_dir = Path(tmp)
    harness_path = tmp_dir / "rabbita_remediation_refresh_receipt_harness.cjs"
...
def assert_workspace_payload() -> None:
  ...
  for margin_id in MARGIN_IDS:
    ...
    if not result["refresh_id"].startswith("refresh-"):
      raise AssertionError(result)
...
def assert_surface(rendered: dict[str, Any]) -> None:
  ...
  if "remediation-margin-refresh-receipt-row" not in row["className"]:
    raise AssertionError(row)
...
def main() -> int:
  ...
  if "Remediation Margin Refresh Receipt" not in html:
    raise AssertionError("missing remediation margin refresh receipt section")
  if "function renderRemediationMarginRefreshReceipt()" not in html:
    raise AssertionError("missing remediation margin refresh receipt renderer")
  ...
  print("checked Rabbita remediation margin refresh receipt surface")
  return 0
