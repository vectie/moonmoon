#!/usr/bin/env python3
"""Execute Rabbita UI and check MoonRobo follow-up modeling evidence."""
...
ENTRY_KIND = "MoonroboRemediationMarginRefreshFollowupModeling"
ENTRY_PATH = (
  "moonrobo/first-trusted-square/remediation-margin-refresh-followup-modeling.json"
)
MODEL_ID = (
...
const rows = document.getElementById('remediation-margin-refresh-followup-modeling').children.map(row => ({
...
"""
  with tempfile.TemporaryDirectory(
    prefix="moonmoon-rabbita-remediation-refresh-followup-modeling-ui-",
  ) as tmp:
    tmp_dir = Path(tmp)
    harness_path = tmp_dir / "rabbita_refresh_followup_modeling_harness.cjs"
...
def assert_workspace_payload() -> None:
  ...
  if primary["source_followup_state"] != "FollowupRefreshesCarriedForward":
    raise AssertionError(primary["source_followup_state"])
  if primary["state"] != "AllFollowupRefreshesStillBlocking":
  ...
  for index, refresh_id in enumerate(REFRESH_IDS, start=1):
    ...
    if result["margin_id"] != MARGIN_IDS[index - 1]:
      raise AssertionError(result)
    if result["result_status"] != "FollowupRefreshStillBlocking":
      raise AssertionError(result)
    if result["refreshed"] is not False:
      raise AssertionError(result)
    if not result["modeling_command"] or not result["evidence_path"]:
      raise AssertionError(result)
...
def assert_surface(rendered: dict[str, Any]) -> None:
  ...
  if "remediation-margin-refresh-followup-modeling-row" not in row["className"]:
    raise AssertionError(row)
...
def main() -> int:
  ...
  if "Remediation Margin Refresh Follow-Up Modeling" not in html:
    raise AssertionError("missing remediation margin refresh follow-up modeling section")
  if "function renderRemediationMarginRefreshFollowupModeling()" not in html:
    raise AssertionError("missing remediation margin refresh follow-up modeling renderer")
  ...
  print("checked Rabbita remediation margin refresh follow-up modeling surface")
  return 0
