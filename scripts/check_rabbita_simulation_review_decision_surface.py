#!/usr/bin/env python3
"""Execute Rabbita UI and check MoonRobo simulation review decision evidence."""
...
ENTRY_KIND = "MoonroboSimulationReviewDecision"
...
const rows = document.getElementById('simulation-review-decision').children.map(row => ({
...
"""
  with tempfile.TemporaryDirectory(prefix="moonmoon-rabbita-sim-decision-ui-") as tmp:
    tmp_dir = Path(tmp)
    harness_path = tmp_dir / "rabbita_sim_decision_harness.cjs"
...
def assert_surface(rendered: dict[str, Any]) -> None:
  ...
  if "simulation-decision-row" not in row["className"]:
    raise AssertionError(row)
...
def main() -> int:
  ...
  if "Simulation Review Decision" not in html:
    raise AssertionError("missing simulation review decision section")
  if "function renderSimulationReviewDecision()" not in html:
    raise AssertionError("missing simulation decision renderer")
  ...
  print("checked Rabbita simulation review decision surface")
  return 0
