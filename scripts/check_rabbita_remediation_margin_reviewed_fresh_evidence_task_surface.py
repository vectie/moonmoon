#!/usr/bin/env python3
"""Execute Rabbita UI and check reviewed fresh-evidence task evidence."""
...
const rows = document.getElementById('remediation-margin-reviewed-fresh-evidence-task').children.map(row => ({
...
"""
  with tempfile.TemporaryDirectory(
    prefix="moonmoon-rabbita-reviewed-fresh-evidence-task-ui-",
  ) as tmp:
    tmp_dir = Path(tmp)
    harness_path = tmp_dir / "rabbita_reviewed_fresh_evidence_task_harness.cjs"
...
def assert_workspace_payload() -> None:
  ...
  if task["source_plan_id"] != PLAN_ID:
    raise AssertionError(task["source_plan_id"])
  if task["source_review_transition_id"] != REVIEW_TRANSITION_ID:
    raise AssertionError(task["source_review_transition_id"])
  if task["source_review_decision"] != "Accept":
    raise AssertionError(task["source_review_decision"])
  ...
  if task["source_receipt_count"] != 3:
  ...
  for margin_id, execution_mode in EXPECTED_ACTIONS.items():
    ...
    if action["result_status"] != "ReviewedWorkItemPendingFreshEvidence":
      raise AssertionError(action)
    if not action["source_receipt_id"].endswith(f"{margin_id}/receipt"):
      raise AssertionError(action)
...
def assert_surface(rendered: dict[str, Any]) -> None:
  ...
  if "remediation-margin-reviewed-fresh-evidence-task-row" not in row["className"]:
    raise AssertionError(row)
...
def main() -> int:
  ...
  if "Remediation Margin Reviewed Fresh Evidence Task" not in html:
    raise AssertionError("missing reviewed fresh evidence task section")
  if "function renderRemediationMarginReviewedFreshEvidenceTask()" not in html:
    raise AssertionError("missing reviewed fresh evidence task renderer")
  ...
  print("checked Rabbita remediation margin reviewed fresh evidence task surface")
  return 0
