#!/usr/bin/env python3
"""Execute Rabbita UI and check MoonRobo refresh projection evidence."""
...
ENTRY_KIND = "MoonroboRemediationMarginRefreshProjection"
...
const rows = document.getElementById('remediation-margin-refresh-projection').children.map(row => ({
...
"""
  with tempfile.TemporaryDirectory(
    prefix="moonmoon-rabbita-remediation-refresh-projection-ui-",
  ) as tmp:
    tmp_dir = Path(tmp)
    harness_path = tmp_dir / "rabbita_remediation_refresh_projection_harness.cjs"
...
def assert_workspace_payload() -> None:
  ...
  if set(payload["consumed_refresh_result_ids"]) != set(REFRESH_IDS):
...
def assert_surface(rendered: dict[str, Any]) -> None:
  ...
  if "remediation-margin-refresh-projection-row" not in row["className"]:
    raise AssertionError(row)
...
def main() -> int:
  ...
  if "Remediation Margin Refresh Projection" not in html:
    raise AssertionError("missing remediation margin refresh projection section")
  if "function renderRemediationMarginRefreshProjection()" not in html:
    raise AssertionError("missing remediation margin refresh projection renderer")
  ...
  print("checked Rabbita remediation margin refresh projection surface")
  return 0
