...
- Add explicit suite adapters later. Moonrobo can provide Noetix/URDF gait and
  ...
  typed authored joint samples generated by Moonrobo. The next implementation
  target is moving the remaining preview-only root/foot motion evidence into
  typed Moonrobo adapter output, then live Moonrobo adapter regeneration for the
  remaining sampled motion evidence.
