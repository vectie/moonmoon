...
- Add explicit suite adapters later. Moonrobo can provide Noetix/URDF gait and
  ...
  typed authored joint, motion, and motor-frame samples generated by Moonrobo.
  The next implementation target is moving the remaining preview-only
  FK/contact evidence into typed Moonrobo adapter output, then live Moonrobo
